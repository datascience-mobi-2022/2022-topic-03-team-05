...
km.out1 = kmeans(data_glob, centers = 2, nstart = 100)
#print(km.out)

# visualize clusters
km.clusters1 = km.out1$cluster
rownames(data_glob) = paste(proteins_global_shift$left_right, 1:nrow(data_glob), sep = "_")
fviz_cluster(list(data = data_glob, cluster = km.clusters1))

# gucken, wie viele left/right shift in welchem Cluster
table(km.clusters1, proteins_global_shift$leftorright)
table(km.clusters1, proteins_global_shift$)
...
diff2 = cbind(numb_max_ctrl, numb_max_rnase)
...
rownames(diff2) = paste(abs_max$left_right, 1:nrow(diff2), sep = "_")
fviz_cluster(list(data = diff1, cluster = km.clusters2), geom = "point")
table(km.clusters2, abs_max$global_max_rnase) ## in cluster 2 there are mostly proteins with rnase maxima in the first ten fractions, in cluster 1 "equally distributed"
table(km.clusters2,abs_max$left_right)

numb = cbind(numb_max_ctrl, numb_max_rnase)
numb_max = cbind(rownames(rep1ctrlnormDataframe), numb)
colnames(numb_max)[1] = "protein"

data1.1 = merge(proteins_global_shift, numb_max, by = "protein")
data1 = data1.1[, c(2,3,12,13)]

# find optimal numbers of clusters
fviz_nbclust(data1, kmeans, method = "wss") + labs(subtitle = "Elbow method")

# k-means

km.out3 = kmeans(data1, centers = 3, nstart = 100)
#print(km.out)

# visualize clusters
km.clusters3 = km.out3$cluster
#rownames(data1) = paste(proteins_global_shift$left_right, 1:nrow(data_glob), sep = "_")
fviz_cluster(list(data = data1, cluster = km.clusters3))
...
data2 = cbind(abs_max[, c(1,2,3,7,8,9,10)], count_shift)
colnames(data2)[3] = "x.shift"
colnames(data2)[4] = "y.shift"
