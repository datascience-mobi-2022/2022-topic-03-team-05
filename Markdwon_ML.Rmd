...
table(km.clusters1, proteins_global_shift$left_right)



diff1 = meanRepctrl - meanRepRNase
diff2 = cbind(rowSums(diff1), count_shift, abs_max$global_max_ctrl, abs_max$global_max_rnase)

fviz_nbclust(diff2, kmeans, method = "wss") + labs(subtitle = "Elbow method")

km.out2 = kmeans(diff2, centers = 2, nstart = 100)
km.clusters2 = km.out2$cluster
rownames(diff2) = paste(proteins_global_shift$left_right, 1:nrow(diff2), sep = "_")
fviz_cluster(list(data = diff2, cluster = km.clusters2))

# gucken, wie viele left/right shift in welchem Cluster
table(km.clusters2, proteins_global_shift$left_right)
```
