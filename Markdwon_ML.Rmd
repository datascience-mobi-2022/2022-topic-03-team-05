...
cor_rep_bad_ctrl = rownames(rep1ctrlnormDataframe) [which(cor_rep_mean_ctrl<0.97)]
length(cor_rep_bad_ctrl) ## 743 der Proteine wären nicht reproduzierbar 
...
length(cor_rep_bad_rnase) ## 893 der Proteine wären nicht reproduzierbar
...
not_repro(loc_max_ctrl3, loc_max_rnase3) # rausschmeißen von 451 Proteinen
not_repro(loc_max_ctrl5, loc_max_rnase5) # rausschmeißen von 371 Proteinen
not_repro(loc_max_ctrl7, loc_max_rnase7) # rausschmeißen von 337 Proteinen
...
for (i in 1:length(loc_max_shift)) {
  j = 1
  while(j <= length(abs_max_shift) && loc_max_shift[i]!=abs_max_shift[j]){
      id[i] = FALSE
      if (j < length(abs_max_shift)) {
        j = j+1
      }
  }
  if(abs_max_shift[j]==loc_max_shift[i]){id[i] = TRUE}
}
```
