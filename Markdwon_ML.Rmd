...
cors_merge = vector()
for(i in 1:nrow(meanRepctrl)){
  cors_merge[i] = cor(meanRepctrl[i,], meanRepRNase[i,], method = "spearman")
}
overview$corr.s = cors_merge
eucli_merge = vector()
for (i in 1:nrow(meanRepctrl)) {
  eu = vector()
  for (j in 1:ncol(meanRepctrl)) {
    eu[j] = sqrt((meanRepctrl[i,j] - meanRepRNase[i,j])^2)
  }
  eucli_merge[i] = eu[which.max(eu)]
}
...
overview$count.shift = ifelse(overview$x.shift.glo == TRUE, overview$count.x.shift.loc + 1, overview$count.x.shift.loc)
...
merge_overview$`no.shift?`= ifelse(merge_overview$`no.shift?`=="shift", TRUE, FALSE)
merge_overview$ymaxdiffc = meanRepctrl[merge_overview$global_max_ctrl]-meanRepRNase[merge_overview$global_max_ctrl]


plot((max_ctrl-max_rnase) ~ corr, data = merge_overview)
cor(merge_overview$x.shift.frac.glo, merge_overview$corr) ## sollte eigentlich niedrig sein
corr_globfrac = lm((ymaxc-ymaxr) ~ (max_ctrl-max_rnase), data = merge_overview)
summary(corr_globfrac)
ggplot(merge_overview,aes(y=x.shift.frac.glo,x=corr))+geom_point()+geom_smooth(method="lm")


plot(m.x.shift.frac.glo ~ m.corr, data = merge_overview)
cor(merge_overview$m.x.shift.frac.glo, merge_overview$m.corr) ## sollte eigentlich niedrig sein
m.corr_globfrac = lm(m.x.shift.frac.glo ~ m.corr, data = merge_overview)
summary(m.corr_globfrac)
ggplot(merge_overview,aes(y=m.x.shift.frac.glo,x=m.corr))+geom_point()+geom_smooth(method="lm")
...
summary(corr_globfrac)
plot(corr_globfrac)
ggplot(train_data, aes(y=x.shift.frac.glo, x=corr, col = left.right, group = 1)) + geom_point() + geom_smooth(method ="lm")


# difference in number of maxima
plot(x.shift.frac.glo ~ (ymaxc - ymaxr), data = train_data)
cor(train_data$x.shift.frac.glo, train_data$ymaxc - train_data$ymaxr) ## sollte eigentlich niedrig sein
maxdiff_globfrac = lm(x.shift.frac.glo ~ (ymaxc - ymaxr), data = test_data)
summary(maxdiff_globfrac)
ggplot(train_data,aes(y=x.shift.frac.glo,x=(ymaxc - ymaxr), col = left.right, group = 1)) + geom_point() + geom_smooth(method="lm") 



# y shift of global maximum of ctrl replicate
plot(x.shift.frac.glo ~ ymaxdiffc, data = train_data)
cor(train_data$x.shift.frac.glo, train_data$ymaxdiffc) ## sollte eigentlich niedrig sein
ydiff_globfrac = lm(x.shift.frac.glo ~ ymaxdiffc, data = test_data)
summary(ydiff_globfrac)
ggplot(train_data,aes(y=x.shift.frac.glo,x=ymaxdiffc, col = left.right, group = 1)) + geom_point() + geom_smooth(method="lm") 
...
ggplot(train_data,aes(y=x.shift.frac.glo,x=eucli, col = left.right, group = 1)) + geom_point() + geom_smooth(method="lm") 



# spearman correlation
plot(x.shift.frac.glo ~ corr.s, data = train_data)
cor(train_data$x.shift.frac.glo, train_data$corr.s) ## sollte eigentlich niedrig sein
corrs_globfrac = lm(x.shift.frac.glo ~ corr.s, data = test_data)
summary(corrs_globfrac)
plot(corrs_globfrac)
ggplot(train_data,aes(y=x.shift.frac.glo,x=corr.s, col = left.right, group = 1)) + geom_point() + geom_smooth(method="lm") 

## Pearson correlation is the most significant feature when predicting the x-shift of the global maximum

## Plot residuals
ggplot(data = train_data, aes(x = corr,y = residuals(corr_globfrac))) + geom_point() + geom_smooth()
...
avg_dis = sqrt(sum((prediction) - test_data$x.shift.frac.glo)^2)/length(test_data$x.shift.frac.glo) 

## avg_dis = comparison of predicted value and actual value -> very small -> good 

par(mfrow=c(1,1))
plot(test_data$x.shift.frac.glo, exp(prediction))
## does not look good?? oder gut?
```
