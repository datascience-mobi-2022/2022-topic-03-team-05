...
```{r}
plot(FullDat_fracnorm$Fraction1_Ctrl_Rep1, FullDat_fracnorm$Fraction1_Ctrl_Rep2)
plot(FullDat_fracnorm$Fraction10_Ctrl_Rep2, FullDat_fracnorm$Fraction1_Ctrl_Rep3)
...
for (j in 1:nrow(rep1ctrlnormDataframe)){
    ...
    c4 = c(as.numeric(rep1rnasenormDataframe[j,]))
    c5 = c(as.numeric(rep2rnasenormDataframe[j,]))
    c6 = c(as.numeric(rep3rnasenormDataframe[j,]))
    cor_rep[j,1] = cor(c1, c2)
    cor_rep[j,2] = cor(c2, c3)
    cor_rep[j,3] = cor(c3, c1)
    cor_rep[j,4] = cor(c4, c5)
    cor_rep[j,5] = cor(c5, c6)
    cor_rep[j,6] = cor(c4, c6)
...
cor_rep_mean_ctrl = apply(cor_rep[,1:3], 1, mean)
cor_rep_bad_ctrl = which(cor_rep_mean<0.97) [which(cor_rep_bad_ctrl<0.97)]
length(cor_rep_bad) ## 364 der Proteine wären nicht reproduzierbar 

cor_rep_mean_rnase = apply(cor_rep[,4:6], 1, mean)
cor_rep_bad_rnase = rownames(rep1ctrlnormDataframe)[which(cor_rep_mean_rnase<0.97)]
length(cor_rep_bad_rnase) ##  der Proteine wären nicht reproduzierbar
...
# zählen der Proteine, die in allen Vergleichen der Reps in RNAs und oder Ctrl mind. eine Fraktion Shift in Nebenmaxima haben
...
#noch in if || (a[i,1] == 0 && a[i,2] != 0 && a[i,3] != 0) || (a[i,1] != 0 && a[i,2] == 0 && a[i,3] != 0) || (a[i,1] != 0 && a[i,2] != 0 && a[i,3] == 0) || (b[i,1] == 0 && b[i,2] != 0 && b[i,3] != 0) || (b[i,1] != 0 && b[i,2] == 0 && b[i,3] != 0) || (b[i,1] != 0 && b[i,2] != 0 && b[i,3] == 0)
# mit denen rausschmeißen, die in einem von beiden zwei Mal Zahlen haben, würde man 3048, 2898, 2430 Proteine rauschmeißen
  
not_repro(loc_max_ctrl3, loc_max_rnase3) # rausschmeißen von 924 Proteinen
not_repro(loc_max_ctrl5, loc_max_rnase5) # rausschmeißen von 776 Proteinen
not_repro(loc_max_ctrl7, loc_max_rnase7) # rausschmeißen von 450 Proteinen
...
verifying normality
```{r}
FullDat_matrix = cbind(rep1ctrlnormData, rep2ctrlnormData, rep3ctrlnormData, rep1rnasenormData, rep2rnasenormData, rep3rnasenormData)


# Sample a random line from the expression table
i = sample(1:nrow(FullDat),1)
# Plot the histogram
hist(FullDat_matrix[i,],
     main=rownames(FullDat)[i],
     col='lightblue',
     breaks=20, 
...
barplot(rep2ctrlnormData)
```


```{r}
# Sample a random line from the expression table
i = sample(1:nrow(FullDat_matrix),1)
#
# Plot the histogram für einzelne Gene
qqnorm(FullDat_matrix[i,],main=rownames(FullDat_matrix)[i]); qqline(FullDat_matrix[i,])
# if normally distributed -> points lie on straight line

shap = 0
for (j in 1:nrow(FullDat_matrix)){
  if ((shapiro.test(FullDat_matrix[j,])$p.value) < 0.05) {shap = shap + 1}
}
shap
shapiro.test(FullDat_matrix[4,])
# Reminder: for p-value >= 0.05, the data is normally distributed. For p-value < 0.05, the data is not normally distributed.

tripctrl_matrix = as.matrix(tripCtrlnormdat)

install.packages('nortest')
library(nortest)
ad = 0
for (i in 1:ncol(tripctrl_matrix)){
  if (ad.test(tripctrl_matrix[,i])$p.value < 0.05) {ad = ad + 1}
}
ad

xnorm = rnorm(1000)
ks.test(xnorm, "pnorm", mean(xnorm), sd(xnorm))
