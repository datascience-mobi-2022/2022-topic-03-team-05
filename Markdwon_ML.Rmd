...
tripCtrlnormdat = tripCtrlnormdat[-c(na_r, na_c),]
tripRnasenormdat = tripRnasenormdat[-c(na_r, na_c),]
# jetzt sind protein 1771, 2359 und 4209 rnase und 639 und 3297 ctrl na
...
cor_rep_bad_ctrl = rownames(rep1ctrlnormDataframe) [which(cor_rep_mean_ctrl<0.95)]
length(cor_rep_bad_ctrl) ## 364 der Proteine wären nicht reproduzierbar 

cor_rep_mean_rnase = apply(cor_rep[,4:6], 1, mean)
cor_rep_bad_rnase = rownames(rep1ctrlnormDataframe)[which(cor_rep_mean_rnase<0.95)]
length(cor_rep_bad_rnase) ## 383 der Proteine wären nicht reproduzierbar
...
sum(thres_3) # rausschmeißen von 502 Proteinen with threshold 3%
thres_5 = not_repro(loc_max_ctrl5, loc_max_rnase5)
sum(thres_5) # rausschmeißen von 411 Proteinen with threshold 5%
thres_7 = not_repro(loc_max_ctrl7, loc_max_rnase7)
sum(thres_7) # rausschmeißen von 365 Proteinen with threshold 7%
...
#compare to corr
repro_loc_max_proteins = rownames(rep1ctrlnormDataframe)[which(thres_5==TRUE)]

id_cor_ctrl = vector()
for (i in 1:length(cor_rep_bad_ctrl)) {
  j = 1
  for(j in 1:length(repro_loc_max_proteins)){
    if(cor_rep_bad_ctrl[i]!=repro_loc_max_proteins[j]){
      id_cor_ctrl[i] = FALSE }
      else if (cor_rep_bad_ctrl[i]==repro_loc_max_proteins[j]) {
        id_cor_ctrl[i] = TRUE
        break
      }
  }
...
id_cor_rnase = vector()
for (i in 1:length(cor_rep_bad_rnase)) {
  j = 1
  for(j in 1:length(repro_loc_max_proteins)){
    if(cor_rep_bad_rnase[i]!=repro_loc_max_proteins[j]){
      id_cor_rnase[i] = FALSE }
      else if (cor_rep_bad_rnase[i]==repro_loc_max_proteins[j]) {
        id_cor_rnase[i] = TRUE
        break
      }
  }
...
loc_max_shift = loc_max_shift[- which(id==TRUE)]

1123+(1348-1123)+(2339-1123) # anzahl shifting proteins
...
if(abs_max$compare==TRUE){
  t.test(abs_max$ymaxc, abs_max_yr)
}

t.test(abs_max[32,4], abs_max[32,5])
t.test(1,2)

t.test(meanRepctrl[1006,], meanRepRNase[1006,], alternative = "two.sided")

h = c(rep1ctrlnormDataframe[1007,23], rep2ctrlnormDataframe[1007,23], rep3ctrlnormDataframe[1007,23])
g = c(rep1rnasenormDataframe[1007,23], rep2rnasenormDataframe[1007,23], rep3rnasenormDataframe[1007,23])

t.test(g,h, alternative = "two.sided")
rep1ctrlnormDataframe[1]
...
# ganz durchlaufen lassen und dann:
# abs_max$pvalue = l
# wenn kleiner 0.05 dann signifikant
# FDR Correction??
