...
set.seed(2)
section = sample(nrow(merge_overview), round(nrow(merge_overview)*0.8))
...
summary(corr_globfrac)
samples = ifelse(train_data$left.right == "no shift", "magenta", "cyan")
ggplot(train_data,aes(y=x.shift.frac.glo,x=corr))+geom_point(col = samples)+geom_smooth(method="lm")
