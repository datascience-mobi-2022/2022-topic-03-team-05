...
for (i in 1:length(loc_max_shift)) {
  j = 1
  for(j in 1:length(cor_rep_bad_ctrl)){
    if(loc_max_shift[i]!=cor_rep_bad_ctrl[j]){
      id_cor_ctrl[i] = FALSE }
      else if (loc_max_shift[i]==cor_rep_bad_ctrl[j]) {
        id_cor_ctrl[i] = TRUE
        break
      }
  }
...
for (i in 1:length(loc_max_shift)) {
  j = 1
  for(j in 1:length(cor_rep_bad_rnase)){
    if(loc_max_shift[i]!=cor_rep_bad_rnase[j]){
      id_cor_rnase[i] = FALSE }
      else if (loc_max_shift[i]==cor_rep_bad_rnase[j]) {
        id_cor_rnase[i] = TRUE
        break
      }
  }
