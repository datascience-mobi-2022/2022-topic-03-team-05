...
diff1 = cbind(data5$max_ctrl, data5$max_rnase)

fviz_nbclust(diff1, kmeans, method = "wss") + labs(subtitle = "Elbow method")

km.out2 = kmeans(diff1, centers = 3, nstart = 100, iter.max = 20)
km.clusters2 = km.out2$cluster

fviz_cluster(list(data = diff1, cluster = km.clusters2), geom = "point")
...
table(km.clusters2,data5$left.right) ## bringt nichts
...
overview$left.right = ifelse(overview$x.shift.glo == TRUE & overview$global_max_ctrl - overview$global_max_rnase > 0, "left", overview$`no.shift?`) 
...
overview$left.right = ifelse(overview$left.right != "left" & overview$left.right != "right" & vectordiff > 0, "right", overview$left.right)
overview$left.right = ifelse(overview$left.right != "left" & overview$left.right != "right" & vectordiff < 0, "left", overview$left.right)
overview$protein = rownames(overview)


data4 = merge(overview, overview_mi, by = "protein")

data4$in_mi = ifelse(data4$left.right == data4$m.left.right, data4$left.right, 0)
data4$in_mi = ifelse(data4$left.right == "right" & data4$m.left.right == "left", "rl", data4$in_mi)
data4$in_mi = ifelse(data4$left.right == "left" & data4$m.left.right == "right", "lr", data4$in_mi)
data4$in_mi = ifelse(data4$left.right == "left" & data4$m.left.right == "no shift", "lno", data4$in_mi)
data4$in_mi = ifelse(data4$left.right == "right" & data4$m.left.right == "no shift", "rno", data4$in_mi)
data4$in_mi = ifelse(data4$left.right == "no shift" & data4$m.left.right == "left", "nol", data4$in_mi)
data4$in_mi = ifelse(data4$left.right == "no shift" & data4$m.left.right == "right", "nor", data4$in_mi)

data3 = cbind(data4$x.shift.glo, data4$y.shift.glo, data4$x.shift.loc, data4$y.shift.loc, data4$m.x.shift.glo, data4$m.y.shift.glo, data4$m.x.shift.glo, data4$m.x.shift.loc)

data3 = cbind(data4$x.shift.frac.glo, data4$m.x.shift.frac.glo, data4$count.x.shift.loc, data4$m.count.x.shift.loc, data4$max_ctrl, data4$max_rnase, data4$m.max_ctrl, data4$m.max_rnase) 

data3 = cbind((data4$global_max_rnase - data4$global_max_ctrl), (data4$m.global_max_rnase - data4$m.global_max_ctrl), data4$max_ctrl, data4$max_rnase, data4$m.max_ctrl, data4$m.max_rnase)

data3 = cbind((data4$global_max_rnase-data4$global_max_ctrl), (data4$m.global_max_rnase-data4$m.global_max_ctrl)) ## bestes bisher!!!!

data3 = cbind((data4$global_max_rnase-data4$global_max_ctrl), (data4$ymaxr-data4$ymaxc))

fviz_nbclust(data3, kmeans, method = "wss") + labs(subtitle = "Elbow method")

km.out3 = kmeans(data3, centers = 4, nstart = 100, iter.max = 20)
...
table(km.clusters3, data4$x.loc.glo) 
table(km.clusters3, data4$y.loc.glo) 
table(km.clusters3, data4$`no.shift?`)
table(km.clusters3, data4$left.right)
table(km.clusters3, data4$y.shift.loc) 
table(km.clusters3, data4$in_mi)


data5 = data4
data5$y.r_c = data5$ymaxr - data5$ymaxc
m = mean(data5$y.r_c)
sd = sd(data5$y.r_c)
data5$y.r_cstandard = (data5$y.r_c - m)/sd
data5$rlnstandard = ifelse(data5$left.right == "left", -1, 0)
data5$rlnstandard = ifelse(data5$left.right == "right", 1, data5$rlnstandard)
data5$m.rlnstandard = ifelse(data5$m.left.right == "left", -1, 0)
data5$rlnstandard = ifelse(data5$m.left.right == "right", 1, data5$m.rlnstandard)
