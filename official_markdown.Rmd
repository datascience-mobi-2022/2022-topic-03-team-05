...
km.out1 = kmeans(kmeans_data1, centers = 4, nstart = 100, iter.max = 20)
...
```
We receive four clusters and check them for the shifting behaviour of the proteins in interphase and mitosis. Cluster 2 contains 171 proteins that shift left in both cell cycle phases indicating that these proteins might be constitutively expressed. In general, we can see a cluster structure, but lying relatively close together, they are not as distinct from each other. Adding the difference in the number of maxima between Ctrl and RNase for interphase and mitosis as additional variables, does not result in a clearer structure. 
