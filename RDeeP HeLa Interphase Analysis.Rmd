...
#kmeans Anzahl maxima und right/left shift

sumctrl = data.frame()
sumrnase = data.frame()
diffshift= data.frame()
vectorsumctrl= rowSums(max_shift_ctrl)
vectorsumrnase = rowSums(max_shift_rnase)

for (i in 7076){
  sumctrl[i,1] = sumctrl[(abs_max[i,1]+ vectorsumctrl[i]),]
  sumrnase[i,1] = sumctrl[(abs_max[i,2]+ vectorsumrnase[i]),]
  diffshift[i,1] = diffshift[sumctrl[i,1]-sumrnase[i,1]]
}
...
for(i in 1:7076){
  sum_m_c = ifelse(max_shift_ctrl[i,1] == 0,FALSE, TRUE)
  sum_max_ctrl = rowSums(sum_m_c)
  
}

l_r_shft_sum = as.data.frame(cbind(diffshift,sum_max_ctrl))
kmeans(l_r_shft_sum, centers = 3, nstart = 10)
