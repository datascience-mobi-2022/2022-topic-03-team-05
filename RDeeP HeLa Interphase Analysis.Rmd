...
}## Bei mir nur voll NA; mean oder Summe? mean von Summe? Weil größere Werte 
...
#wenn FullDat_fracnorm bei mir richtig ist: FullDat = FullDat_fracnorm, damit damit weitergearbeitet wird
## data sets for each replicate; normalization, row-wise
...
compare_max(maxctrl1_3,maxctrl2_3) #1327
compare_max(maxctrl1_3,maxctrl3_3)#1861
compare_max(maxctrl2_3,maxctrl3_3) #1565

compare_max(maxctrl1_5,maxctrl2_5) #1172
compare_max(maxctrl1_5,maxctrl3_5) #1635
compare_max(maxctrl2_5,maxctrl3_5) #1395

compare_max(maxctrl1_7,maxctrl2_7)#990
compare_max(maxctrl1_7,maxctrl3_7) #1492
compare_max(maxctrl2_7,maxctrl3_7) #1305

#in which fractions are all maxima between all three replicates different -> in none ?
allfrac_diff = matrix(0, ncol = 25, nrow = 7081) 
for (i in 1:7081){
  for (j in 2:24){
    if(maxctrl1_3[i,j]!=maxctrl2_3[i,j] && maxctrl1_3[i,j]!= maxctrl3_3[i,j] && maxctrl2_3[i,j]!= maxctrl3_3[i,j]){allfrac_diff[i,j]= 1}
    }
}

which(allfrac_diff==TRUE)
