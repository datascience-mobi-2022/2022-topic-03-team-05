...
The amount k of centers is pre-defined, each point is the assigned to the point closest to the centers. For each cluster the center of gravity is determined and the points are assigned to the closest center again. This process is repeated xxxxxxxxxxxx (Teil raus machen? Noch mehr kürzen?) up to when a certain number of maximum iterations is reached, or when no point should be repeated.xxxxxxxxxx Fluctuations indicate the lack of a clear cluster structure, however this method identifies clusters, regardless of their biological significance. 
...
After Data reduction, normalization and removal of batch effects we used tested for reproducibility. 411 proteins that did not meet our criteria were removed, as this difference in protein amount might have been due to experimental errors. Due to the verification of the reproducibility between replicates we assumed normality of the data which was used for statistical analysis.
...
To avoid overinterpreation or false-positives we then performed a t-test on the shift in the y-direction to conclude if the shift is significant. This t-test xxxxxxxxxxxx (Satz kann man weglassen weil unten Ergebnisse t-test kommen,oder?)
...
Following the result of the elbow plot which showed a kink the number of four clusters, the k-means clustering led to four different clusters. They lie relatively close together due to similar input variable values, but are distinct from each other. When looking at tabels that gave us the number of proteins with a left shift, right shift and a shift in the local maxima as additional info, it stands out that cluster 2 contains 171 RBPs that shift left in both cell cycle phases. In cluster 1 there are proteins that show the characteristic of an RBP during mitosis, but split up to all different kinds of categories of our shift analysis during interphase. For cluster 3 it behaves the opposite way. Adding the difference in the number of maxima between Ctrl and RNase for interphase and mitosis as additional variables, does not result in a clearer structure. The last cluster cannot be interpretated that well. It contains most of the proteins that do not shift (no RBPs). 

xxxxxxxxxxxx Plot k-means Figure x: k-means clustering. The shift of the global maximum of the proteins during interphase was clustered against the shift during mitosis to compare the RNA interactome between the cell cycle phases.
...
xxxxxxxxxxxxxxxx Plot regression Figure x: Linear regression analysis. The absolute value of the x-shift of the global maximum was predicted with the correlation of the averaged curves for RNase and control. The plot shows the data points and the regression line (y = 14.21x - 14.29)
