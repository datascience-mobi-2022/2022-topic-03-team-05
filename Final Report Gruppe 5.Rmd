...
After Data reduction, normalization and removal of batch effects we used tested for reproducibility. 411 proteins that did not meet our criteria were removed from the Interphase dataset, as this difference in protein amount might have been due to experimental errors. Due to the verification of the reproducibility between replicates we assumed normality of the data which was used for statistical analysis.
...
The k-means clustering resulted in four different clusters. The 171 proteins shifting left in both cell cycle phases are probably proteins that are constitutively expressed with the same function and binding behaviour in interphase and mitosis. It would be interesting to further examine these proteins concerning features and functions to find out if and what they have in common. Proteins in cluster 1 and 3 differ in their properties of RNA-binding dependent on the cell cycle phase. We clustered one variable against one other variable, however adding additional variables might improve the cluster structure and the outcome. So we tried adding more information, without improvement indicating that there is no strong relation between the number of maxima and the x-shift of the global maximum. Combining other variables would have probably led to clearer clusters. As cluster 4 contains a lot of no shifting proteins, we hypothesize that shifting proteins lying in this cluster show a smaller number of fractions their global maximum shifted, explaining why they are clustered together with the ones without a shift.  
