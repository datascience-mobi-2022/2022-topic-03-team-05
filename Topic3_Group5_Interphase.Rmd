...
for (i in colRep1ctrl){
  barplot(rep_sum[i:(i+2)], main = "protein amount per Ctrl replicate before normalization", cex.names = .8)}

## RNase sum fraction-wise before normalization
for (i in colRep1rnase){
  barplot(rep_sum[i:(i+2)], main = "protein amount per RNase replicate before normalization", cex.names = .8)}
...
for (i in colRep1ctrl){
  barplot(rep_sum_norm[i:(i+2)], main = "protein amount per Ctrl replicate after normalization", cex.names = .8)}

## RNase sum fraction-wise before normalization and visualize protein amount
for (i in colRep1rnase){
  barplot(rep_sum_norm[i:(i+2)], main = "protein amount per RNase replicate after normalization", cex.names = .8)
}
...
sum(thres_7) ## we would remove 361 proteins with threshold 7%
...
sum(id) ## 913 proteins are both found in absolute and local maxima x-shifts
loc_max_shift = loc_max_shift[- which(id==TRUE)] ## loc_max_shift are all proteins x-shifting in local maximum excluded those also shifting in global maximum (989)
...
```
We find 2107 proteins shifting in their global and/or local maxima. In the following, we test if the shift in protein amount is significant which would result in classifying the protein as an RNA-binding or RNA-dependent protein. 
...
By using k-means we aim at identifying specific groups within the data sets. First, we look for clusters of proteins when clustering the difference in fraction of global maximum in Ctrl and RNase of interphase with the difference in fraction of global maximum in Ctrl and RNase of mitosis. The idea of this approach is that proteins might be assigned to the same cluster if shifting in a similar way/ not in a similar way. As the clustering might be more significant the more variables are used as input, a second step constists of adding the difference in number of maxima between Ctrl and RNase for interphase and mitosis as additional information and check if the cluster structure improves. 
...
kmeans_data1 = as.data.frame(cbind((merge_overview$global_max_ctrl-merge_overview$global_max_rnase), (merge_overview$m.global_max_ctrl-merge_overview$m.global_max_rnase))) 
colnames(kmeans_data1) = c("x.shift.interphase", "x.shift.mitosis")
...
plot(1:8,wss1,type='b',pch=19,xlab="Number of clusters K", ylab="Total within-clusters sum of squares", main = "Elbow plot") ## elbow plot would suggest 4 clusters 
...
kmeans_data2 = as.data.frame(cbind((merge_overview$global_max_ctrl-merge_overview$global_max_rnase), (merge_overview$m.global_max_ctrl-merge_overview$m.global_max_rnase), (merge_overview$count.shift), (merge_overview$m.count.shift)))
...
plot(1:8,wss2,type='b',pch=19,xlab="Number of clusters K",
     ylab="Total within-clusters sum of squares", main = "Elbow plot") ## elbow plot would suggest 3 clusters

## perform k-means and visualize clusters
km.out2 = kmeans(kmeans_data2, centers = 3, nstart = 100)
...
## no improvement in cluster structure due to additional variable (higher total wws for first clustering approach)
```
We receive four clusters and check them for the shifting behaviour of the proteins in interphase and mitosis. One cluster contains 171 proteins that shift left in both cell cycle phases indicating that these proteins might be constitutively expressed. In general, we can see a cluster structure, but lying relatively close together, they are not as distinct from each other. Adding the number of shifts of maxima for interphase and mitosis as additional variables, does not result in a clearer structure which was tested by computing the total within-cluster sum of squares. 

# Regression analysis

For performing a linear regression, we split our data set into two sets, the training and the test data (80 %/ 20%). Our goal is to predict the number of fractions the global maximum of a given protein shifts. We compared two different information as explanatory variables: the Pearson correlation of the mean curve of control and RNase and the highest euclidean distance between the two average curves. After that, the x shift is predicted for the test data set using the more significant explanatory variable. The model can be evaluated looking at the R^2 value and the p-value of the F-test. 
...
## test conditions for linear regression: residuals are normally distributed and no correlation between residuals and the explanatory variable
...
```
The x-shift of the global maximum can be approximated using the correlation between the average Ctrl and RNase curve. The model explains 68.06 % of the total variance, so relatively good. Testing it on the data set test_data results in a root mean square error of 1.91. 
