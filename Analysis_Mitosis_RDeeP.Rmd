...
rep1ctrldat = mFullDat[,colRep1ctrl] 
rep2ctrldat = mFullDat[,colRep2ctrl]
rep3ctrldat = mFullDat[,colRep3ctrl]
rep1rnasedat = mFullDat[,colRep1rnase]
rep2rnasedat = mFullDat[,colRep2rnase]
rep3rnasedat = mFullDat[,colRep3rnase]
```
...
```{r}
remo1 = which(rowSums(is.na(TripCtrldat))==75) ##ist 0
remo2 = which(rowSums(is.na(TripRnasedat))==75) ##ist 0
mFullDat = mFullDat[-c(remo1,remo2),]
...
mean_frac_rep = apply(mFullDat, 2, mean)
sd_frac_rep = apply(mFullDat,2, sd)
...
```{r}
rep_sum_norm = apply(mFullDat_fracnorm, 2, sum)
#Ctrl sum fraction-wise before normalization
for (i in colRep1ctrl){
...
rep_mean_norm = apply(mFullDat_fracnorm, 2, mean)
for (i in colRep1ctrl){
...
rep1ctrlnormData = t(apply(mFullDat[, colRep1ctrl], 1, function(x) x/(sum(x)/100)))
rep2ctrlnormData = t(apply(mFullDat[, colRep2ctrl], 1, function(x) x/(sum(x)/100)))
rep3ctrlnormData = t(apply(mFullDat[, colRep3ctrl], 1, function(x) x/(sum(x)/100)))
rep1rnasenormData = t(apply(mFullDat[, colRep1rnase], 1, function(x) x/(sum(x)/100)))
rep2rnasenormData = t(apply(mFullDat[, colRep2rnase], 1, function(x) x/(sum(x)/100)))
rep3rnasenormData = t(apply(mFullDat[, colRep3rnase], 1, function(x) x/(sum(x)/100)))
...
#mFullDat = mFullDat[-na_afterbatch,]
...
sum(thres_3) # remove 594 proteins with threshold 3%
thres_5 = not_repro(loc_max_ctrl5, loc_max_rnase5)
sum(thres_5) # remove 498 proteins with threshold 5%
thres_7 = not_repro(loc_max_ctrl7, loc_max_rnase7)
sum(thres_7) # remove 303 proteins with threshold 7% (wahrscheinlich jetzt anders)

# decide to go with maximum has to contain at least 5 % of total protein amount
# remove proteins that are not reproducible
mFullDat = mFullDat[- which(thres_5 == TRUE),]
