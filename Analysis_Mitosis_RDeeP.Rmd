...
#load("mFullDat.Rdata") ##nur den ausführen bei R-Datei von jemand anderem
dim(mFullDat)
```
...
mFullDat[mFullDat==0] = NA ##würde ich rausnehmen und nur für mean und co vllt einbauen (aplly hat kein rm.na!!!)
...
```{r}
mremo1 = which(rowSums(is.na(mTripCtrldat))==75) ##ist 0
mremo2 = which(rowSums(is.na(mTripRnasedat))==75) ##ist 0
mFullDat = mFullDat[-c(mremo1,mremo2),]
...
sum(mthres_3) # remove 594 proteins with threshold 3%
mthres_5 = not_repro(mloc_max_ctrl5, mloc_max_rnase5)
sum(mthres_5) # remove 498 proteins with threshold 5%
mthres_7 = not_repro(mloc_max_ctrl7, mloc_max_rnase7)
sum(mthres_7) # remove 303 proteins with threshold 7% (wahrscheinlich jetzt anders)
...
row.names(mmax_shift_ctrl) = rownames(mrep1ctrlnormDataframe)
row.names(mmax_shift_rnase) = rownames(mrep1ctrlnormDataframe)
mmax_shift_ctrl = as.data.frame(mmax_shift_ctrl)
mmax_shift_rnase = as.data.frame(mmax_shift_rnase)
mmax_shift_ctrl$count = mcount_shift
mmax_shift_rnase$count = mcount_shift
#mmax_shift_ctrl = mmax_shift_ctrl[which(mmax_shift_ctrl$count > 0),]
#mmax_shift_rnase = mmax_shift_rnase[which(mmax_shift_rnase$count > 0),]
#mproteins_local_shift = as.data.frame(cbind(mmax_shift_ctrl, mmax_shift_rnase))
#mproteins_local_shift$count = mcount_shift
...
sum(mid)
# 772 proteins are both found in absolute and local maxima shifts
mloc_max_shift = mloc_max_shift[- which(mid==TRUE)] ## mloc_max_shift are all proteins shifting in local maximum excluded those also shifting in global maximum
...
sum(mabs_max$sign) ## how many proteins have a significant shift in protein amount in their global maximum 

colnames(mabs_max)[1] = "m.global_max_ctrl"
colnames(mabs_max)[2] = "m.global_max_rnase"
mproteins_global_shift = data.frame(mabs_max[(which(mabs_max$sign == TRUE)),])
...
mmax_shift_ctrl = mmax_shift_ctrl[,-26]
mmax_shift_rnase = mmax_shift_rnase[,-26]

mp_loc_max = matrix(10, ncol = 25, nrow = nrow(mrep1ctrlnormDataframe))
row.names(mp_loc_max) = rownames(mrep1ctrlnormDataframe)
mp_loc_shift = vector(length = nrow(mrep1ctrlnormDataframe))
msign_loc_shiftc = matrix(0, ncol = 25, nrow = nrow(mrep1ctrlnormDataframe))
msign_loc_shiftr = matrix(0, ncol = 25, nrow = nrow(mrep1ctrlnormDataframe))
...
mproteins_both_shift = merge(mproteins_global_shift, mproteins_local_shift, by = "protein") ## data sets merged by row names
...
mproteins_shifting = merge(mproteins_global_shift, mproteins_local_shift, by = "protein", all = TRUE) ## 1653 proteins have a shift in either local and/or global maximum
mproteins_shifting = mproteins_shifting[,-(12:36)]
# get names with mproteins_shifting[,1]
...
library(factoextra)
library(ggplot2)
mdata_glob = mproteins_global_shift[,c(2:3)]

# find optimal numbers of clusters
fviz_nbclust(mdata_glob, kmeans, method = "wss") + labs(subtitle = "Elbow method")

# k-means

mkm.out1 = kmeans(mdata_glob, centers = 4, nstart = 100)
#print(km.out)

# visualize clusters
mkm.clusters1 = mkm.out1$cluster
rownames(mdata_glob) = paste(mproteins_global_shift$left_right, 1:nrow(mdata_glob), sep = "_")
fviz_cluster(list(data = mdata_glob, cluster = mkm.clusters1), geom = "point")
#for ggplot mdata_glob$cluster = as.character(mkm.out1$cluster)
#ggplot() +
#  geom_point(data = mdata_glob,
#            mapping = aes(x = global_max_ctrl, y = global_max_rnase, colour = cluster))

# gucken, wie viele left/right shift in welchem Cluster
table(mkm.clusters1, mproteins_global_shift$leftorright) ## right mostly in cluster 2 and 3, left in cluster 1 and 4
table(mkm.clusters1, mproteins_global_shift$m.global_max_rnase) ## cluster 4 first ten fractions, cluster 1 middle fractions
table(mkm.clusters1, mproteins_global_shift$m.global_max_ctrl) ## cluster 1 last ten fractions, cluster 4 last ten fractions





mnumb_max_ctrl = rowSums(mmax_shift_ctrl!=0)+1 ## plus global maximum
mnumb_max_rnase = rowSums(mmax_shift_rnase!=0)+1 ## plus global maximum

mabs_max$max_ctrl = mnumb_max_ctrl
mabs_max$max_rnase = mnumb_max_rnase
mabs_max$left_right = ifelse(mp_loc_shift == TRUE | mabs_max$sign == TRUE, "shift", "no shift") 
mdiff1 = mabs_max[,c(1:2,8,9)]

fviz_nbclust(mdiff1, kmeans, method = "wss") + labs(subtitle = "Elbow method")

mkm.out2 = kmeans(mdiff1, centers = 2, nstart = 100, iter.max = 20)
mkm.clusters2 = mkm.out2$cluster
fviz_cluster(list(data = mdiff1, cluster = mkm.clusters2), geom = "point") ## looks weird
table(mkm.clusters2, mabs_max$global_max_rnase) ## in cluster 1 there are mostly proteins with rnase maxima in the first 10 fractions, in cluster 2 "equally distributed"
table(mkm.clusters2,mabs_max$left_right) ## bringt nichts
...
colnames(overview_mi)[3] = "m.x.shift.glo"
colnames(overview_mi)[6] = "m.y.shift.glo"
colnames(overview_mi)[9] = "m.no.shift?"
colnames(overview_mi)[10] = "m.count.x.shift.loc"
colnames(overview_mi)[11] = "m.y.shift.loc"
colnames(overview_mi)[4] = "m.ymaxc"
colnames(overview_mi)[5] = "m.ymaxr"
colnames(overview_mi)[7] = "m.max_ctrl"
colnames(overview_mi)[8] = "m.max_rnase"
overview_mi$m.x.shift.loc = ifelse(overview_mi$m.count.x.shift.loc != 0, TRUE, FALSE)
overview_mi$m.x.loc.glo = ifelse(overview_mi$m.x.shift.glo == TRUE & overview_mi$m.x.shift.loc == TRUE, TRUE, FALSE)
overview_mi$m.y.loc.glo = ifelse(overview_mi$m.y.shift.glo == TRUE & overview_mi$m.y.shift.loc == TRUE, TRUE, FALSE)
...
overview_mi$m.x.shift.frac.glo = overview_mi$m.global_max_rnase - overview_mi$m.global_max_ctrl

overview_mi$protein = rownames(overview_mi)

mvgl_locshiftc = msign_loc_shiftc
mvgl_locshiftc[mvgl_locshiftc != 0] = TRUE
mvgl_locshiftr = msign_loc_shiftr
mvgl_locshiftr[mvgl_locshiftr != 0] = TRUE
mvectorsumctrl= rowSums(mvgl_locshiftc)
mvectorsumrnase = rowSums(mvgl_locshiftr)
mvectordiff = mvectorsumrnase - mvectorsumctrl ## count local maxima 
mvectorlocshift = rowSums(msign_loc_shiftc[which(mvectordiff==0),]) - rowSums(msign_loc_shiftr[which(mvectordiff==0),])



mloc_overview = overview_mi[which(mvectordiff==0),]

mloc_overview$m.left.right = ifelse(mloc_overview$m.left.right != "left" & mloc_overview$m.left.right != "right" & mvectorlocshift > 0, "left", mloc_overview$m.left.right)
mloc_overview$m.left.right = ifelse(mloc_overview$m.left.right != "left" & mloc_overview$m.left.right != "right" & mvectorlocshift < 0, "right", mloc_overview$m.left.right)
mloc_overview$protein = rownames(mloc_overview)
## data set with all proteins that have the same number of maxima in ctrl and RNase



data3 = overview_mi[,c(3,7,8,10)]
# 3,7,8,10 ergibt 3 cluster ganz ok

fviz_nbclust(data3, kmeans, method = "wss") + labs(subtitle = "Elbow method")

km.out3 = kmeans(data3, centers = 3, nstart = 100, iter.max = 20)
km.clusters3 = km.out3$cluster
#rownames(data3) = paste(overview_mi$left.right, 1:nrow(diff2), sep = "_")
...
fviz_cluster(list(data = data3, cluster = km.clusters3), geom = "point")
table(km.clusters3, overview_mi$x.loc.glo) 
table(km.clusters3, overview_mi$y.loc.glo) 
table(km.clusters3, overview_mi$`no.shift?`)
table(km.clusters3, overview_mi$left.right)
table(km.clusters3, overview_mi$y.shift.loc)
