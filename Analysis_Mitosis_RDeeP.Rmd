...
mthres_3 = not_repro(mloc_max_ctrl3, loc_max_rnase3)
sum(mthres_3) # remove 594 proteins with threshold 3%
mthres_5 = not_repro(mloc_max_ctrl5, loc_max_rnase5)
sum(mthres_5) # remove 498 proteins with threshold 5%
mthres_7 = not_repro(mloc_max_ctrl7, loc_max_rnase7)
...
meanRepctrl = matrix(nrow= nrow(mFullDat), ncol = 25)
for (j in 1:nrow(mFullDat)){ 
  for (i in 1:25){
    meanRepctrl[j,i] = (mrep1ctrlnormDataframe[j,i] + mrep2ctrlnormDataframe[j,i] + mrep3ctrlnormDataframe[j,i])/3}}

#mean of rnase replicates of all proteins 
meanRepRNase = matrix(nrow= nrow(mFullDat), ncol = 25)
for (j in 1:nrow(mFullDat)){ 
  for (i in 1:25){
    meanRepRNase[j,i] = (mrep1rnasenormDataframe[j,i] + mrep2rnasenormDataframe[j,i] + mrep3rnasenormDataframe[j,i])/3}}
...
```{r}
# absolute maxima
abs_max = data.frame(row.names = rownames(mrep1ctrlnormDataframe))
...
max_shift_ctrl = matrix(0, nrow = nrow(mrep1ctrlnormDataframe), ncol = 25)
max_shift_rnase = matrix(0, nrow = nrow(mrep1ctrlnormDataframe), ncol = 25)
...
loc_max_shift = rownames(mrep1ctrlnormDataframe)[which(count_shift!=0)] ## x-shift
row.names(max_shift_ctrl) = rownames(mrep1ctrlnormDataframe)
row.names(max_shift_rnase) = rownames(mrep1ctrlnormDataframe)
...
sum(id)+(length(abs_max_shift)-sum(id))+length(loc_max_shift) ## 1828 proteins have a shift and might be RNA-binding or RNA-dependent
...
for (i in 1:nrow(abs_max)) { 
  if(abs_max[i,3]==TRUE) {
    fc = abs_max[i,1]
    l[i] = t.test(c(mtripCtrlnormdat[i,fc], mtripCtrlnormdat[i, (fc+25)], mtripCtrlnormdat[i, (fc+50)]), c(mtripRnasenormdat[i,fc], mtripRnasenormdat[i, (fc+25)], mtripRnasenormdat[i, (fc+50)]), alternative = "two.sided")$p.value
  }
...
sum(abs_max$sign) ## how many proteins have a significant shift in protein amount in their global maximum 

colnames(abs_max)[1] = "m.global_max_ctrl"
colnames(abs_max)[2] = "m.global_max_rnase"
...
sum(proteins_global_shift$sign_ad) ## all 641 proteins with shift identified are still significant after fdr-correction

proteins_global_shift$leftorright = ifelse(proteins_global_shift$m.global_max_ctrl < proteins_global_shift$m.global_max_rnase, "right", "left")
...
p_loc_max = matrix(10, ncol = 25, nrow = nrow(mrep1ctrlnormDataframe))
row.names(p_loc_max) = rownames(mrep1ctrlnormDataframe)
p_loc_shift = vector(length = nrow(mrep1ctrlnormDataframe))
sign_loc_shiftc = matrix(0, ncol = 25, nrow = nrow(mrep1ctrlnormDataframe))
sign_loc_shiftr = matrix(0, ncol = 25, nrow = nrow(mrep1ctrlnormDataframe))
    ...
    for (j in 1) {
      if(max_shift_ctrl[i,j]==max_shift_rnase[i,j]){
          ...
          if(max_shift_ctrl[i,(j+1)] != 0){
              ...
              p_loc_max[i,j] = t.test(c(mtripCtrlnormdat[i,j], mtripCtrlnormdat[i, (j+25)], mtripCtrlnormdat[i, (j+50)]), c(mtripRnasenormdat[i,j], mtripRnasenormdat[i, (j+25)], mtripRnasenormdat[i, (j+50)]), alternative = "two.sided")$p.value
...
nrow(proteins_only_global) ## 163 proteins only shift in global maximum
proteins_only_local = anti_join(proteins_local_shift, proteins_both_shift, by = "protein") 
nrow(proteins_only_local) ## 603 proteins only shift in local maximum
...
km.out1 = kmeans(data_glob, centers = 4, nstart = 100)
#print(km.out)
...
table(km.clusters1, proteins_global_shift$leftorright) ## right mostly in cluster 2 and 3, left in cluster 1 and 4
table(km.clusters1, proteins_global_shift$m.global_max_rnase) ## cluster 4 first ten fractions, cluster 1 middle fractions
table(km.clusters1, proteins_global_shift$m.global_max_ctrl) ## cluster 1 last ten fractions, cluster 4 last ten fractions
...
diff1 = abs_max[,c(1:2,8,9)]

fviz_nbclust(diff1, kmeans, method = "wss") + labs(subtitle = "Elbow method")

km.out2 = kmeans(diff1, centers = 2, nstart = 100, iter.max = 20)
km.clusters2 = km.out2$cluster
fviz_cluster(list(data = diff1, cluster = km.clusters2), geom = "point") ## looks weird
table(km.clusters2, abs_max$global_max_rnase) ## in cluster 1 there are mostly proteins with rnase maxima in the first 10 fractions, in cluster 2 "equally distributed"
table(km.clusters2,abs_max$left_right) ## bringt nichts



# anzahl maxima und anzahl shift gegen shift in haupt, haupt+neben oder nur nebenmaximum

overview_mi = cbind(abs_max[, c(1,2,3,4,5,7,8,9,10)], count_shift, p_loc_shift)
...
overview_mi$m.left.right = ifelse(overview_mi$m.x.shift.glo == TRUE & overview_mi$m.global_max_ctrl - overview_mi$m.global_max_rnase > 0, "left", overview_mi$`m.no.shift?`) 
...
vectorsumctrl= rowSums(sign_loc_shiftc)
vectorsumrnase = rowSums(sign_loc_shiftr)
vectordiff = vectorsumrnase - vectorsumctrl

overview_mi$m.left.right = ifelse(overview_mi$m.left.right != "left" & overview_mi$m.left.right != "right" & vectordiff > 0, "right", overview_mi$m.left.right)
overview_mi$m.left.right = ifelse(overview_mi$m.left.right != "left" & overview_mi$m.left.right != "right" & vectordiff < 0, "left", overview_mi$m.left.right)
