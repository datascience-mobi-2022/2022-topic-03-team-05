...
tcolRep1ctrl = seq(1, 150, 6) ##all columns with measurements from rep1 control sample 
tcolRep2ctrl = seq(2, 150, 6) ##all columns with measurements from rep2 control sample
tcolRep3ctrl = seq(3, 150, 6) ##all columns with measurements from rep3 control sample
tcolRep1rnase = seq(4, 150, 6) ##all columns with measurements from rep1 rnase sample
tcolRep2rnase = seq(5, 150, 6) ##all columns with measurements from rep2 rnase sample
tcolRep3rnase = seq(6, 150, 6) ##all columns with measurements from rep3 rnase sample
...
plot(datprot1, main = "amount of 1433B_HUMAN in mean of control sample replicates", xlab = "fractions of sucrose density gradient", ylab = "percentage of protein amount", col = "blue"); lines(datprot1, lwd=2, col = "blue"); lines(x, l, col = "magenta", lwd=2)
```

plot suchen und gefunden für mit shift
```{r}
for (i in 3927:3927) {
  plot(y, rep1ctrlnormData[i,], col = "black", type = "n", main = "amount of NOP9_HUMAN (all 6 replicates)", xlab = "fractions of sucrose density gradient", ylab = "percentage of protein amount") ##plot aller samples für protein 900-930 normalisiert auf 100 (wenn es geklappt hat)
...
hist(countnas[1:100], main = "NAs in Replicate 1 of control sample", xlab = "number of NAs", breaks = seq(0,25,1))
...
```{r}
for (i in 413:415) {
  plot(y, rep1ctrlnormData[i,], col = "black", type = "n", main = "amount of ARI1A_HUMAN (all 6 replicates)", xlab = "fractions of sucrose density gradient", ylab = "percentage of protein amount") ##plot aller samples für protein 900-930 normalisiert auf 100 (wenn es geklappt hat)
  lines(y,rep1ctrlnormData[i,], col = "black")
  lines(y, rep2ctrlnormData[i,], col = "black") ##plot des Rep2 ctrl sample für protein 1
  lines(y, rep3ctrlnormData[i,], col = "black") ##plot des Rep3 ctrl sample für protein 1
  lines(y, rep1rnasenormData[i,], col = "magenta")
